...
def predict_image(sess, model, image, colors_dict):
    ...
    segmented_image_2 = np.argmax(predicted_class,axis=2)
    ...
    for label in range(len(colors_dict)):
        ...
        #segmentation = predicted_class
        #mask = segmentation
        #result_im = scipy.misc.toimage(mask, mode="RGBA")
    ...
    out2 = segmented_image_2
    #result_im = scipy.misc.toimage(image)

    return out2, tf_time_ms, img_time_ms
...
def predict_files(args, image_shape):
    ...
    with tf.Session(config=session_config(args)) as sess:
        ...
        for image_file in images_pbar:
            ...
            segmented_image = cv2.resize(segmented_image,(2048, 1024), interpolation = cv2.INTER_NEAREST)
            scipy.misc.toimage(segmented_image, cmin=0, cmax=255, mode='I').save(os.path.join(output_dir, os.path.basename(image_file)))
            #scipy.misc.imsave(os.path.join(output_dir, os.path.basename(image_file)), segmented_image_unscaled)
...
if __name__ == '__main__':
    ...
    test_images_path_pattern = '../cityscapes/data/leftImg8bit/val/*/*.png'
